...
BEGIN
  ...
  LOOP
    ...
    -- Gate: empty seats pick immediately; human seats wait for deadline.
    IF NOT _is_empty_seat THEN
      IF _room.pick_deadline IS NULL OR now() < _room.pick_deadline THEN
        CONTINUE;
      END IF;
    END IF;

    -- 1) For a real user, try their queue first (skip drafted players)
    _player_id := NULL;
    ...
    -- 2) Positional-need fallback: figure out which starting slots are unfilled
    IF _player_id IS NULL THEN
      ...
      -- Try best available that fills an unfilled starting slot
    ...
    -- 3) Final fallback: best available overall
    IF _player_id IS NULL THEN
      SELECT
        COALESCE(p.loose_key, p.player_key),
        p.full_name,
    ...
    -- Advance. For empty-seat picks we set pick_deadline = now() so the next
    -- tick fires immediately and chains through consecutive empty seats fast.
    ...
    ELSE
      ...
      SET current_pick_number = _room.current_pick_number + 1,
          pick_deadline = CASE
            WHEN _is_empty_seat THEN now()  -- fire ASAP for next tick
