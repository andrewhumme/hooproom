-- 1) Fix array-concat in snake_autopick_due (|| 'SF' was ambiguous and threw
--    "malformed array literal"). Use array-to-array concat.
...
BEGIN
  ...
  LOOP
    ...
    -- Final fallback: best available regardless of position
    IF _player_id IS NULL THEN
      SELECT
        COALESCE(p.loose_key, p.player_key), p.full_name, p.position, p.team_abbreviation
        INTO _player_id, _player_name, _player_position, _player_team
...
-- 2) Replace the HTTP-based cron jobs with direct in-DB calls
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'snake-autopick-tick') THEN
    PERFORM cron.unschedule('snake-autopick-tick');
  END IF;
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'snake-autopick-tick-direct') THEN
    PERFORM cron.unschedule('snake-autopick-tick-direct');
  END IF;
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'auction-tick-direct') THEN
    PERFORM cron.unschedule('auction-tick-direct');
  END IF;
END $$;

SELECT cron.schedule(
  'snake-autopick-tick-direct',
  '10 seconds',
  $cron$ SELECT public.snake_autopick_due(); $cron$
);
...
SELECT cron.schedule(
  ...
  '5 seconds',
  $cron$
    SELECT public.auction_award_due(NULL);
    SELECT public.auction_bot_bid_due();
    SELECT public.auction_bot_nominate_due();
    SELECT public.lobby_autostart_due();
  $cron$
);

-- 3) Kick once to flush any currently-overdue picks
SELECT public.snake_autopick_due();
SELECT public.auction_award_due(NULL);