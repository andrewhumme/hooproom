...
gr AS (
  SELECT h.loose_key AS lk,
         LEAST(1.18, GREATEST(0.95, 1.0
           + CASE COUNT(DISTINCT h.season)
               WHEN 1 THEN 0.10 WHEN 2 THEN 0.07 WHEN 3 THEN 0.04 WHEN 4 THEN 0.02
               ELSE 0 END
           + COALESCE(GREATEST(-0.04, LEAST(0.08,
               ((MAX(CASE WHEN h.season = 2026 THEN h.minutes_per_game END)
                 - MAX(CASE WHEN h.season = 2025 THEN h.minutes_per_game END))
                / NULLIF(MAX(CASE WHEN h.season = 2025 AND h.minutes_per_game > 8 THEN h.minutes_per_game END),0)
               ) * 0.5)), 0)
         ))::numeric AS growth
  FROM public.player_season_stats h
  WHERE h.season BETWEEN 2021 AND 2026
    AND h.loose_key IS NOT NULL
    AND COALESCE(h.games_played,0) > 0
  GROUP BY h.loose_key
),
base AS (
  SELECT s.loose_key AS lk,
         (COALESCE(av.avail, 1.0) * COALESCE(gr.growth, 1.0)) AS f,
         COALESCE(s.pts,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS pts,
         COALESCE(s.reb,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS reb,
         COALESCE(s.ast,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS ast,
         COALESCE(s.stl,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS stl,
         COALESCE(s.blk,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS blk,
         COALESCE(s.tov,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS tov,
         COALESCE(s.fg3_made,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS fg3m,
         COALESCE(s.fg_pct,0)::numeric AS fg_pct,
         COALESCE(s.fg_att,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS fg_att,
         COALESCE(s.ft_pct,0)::numeric AS ft_pct,
         COALESCE(s.ft_att,0)::numeric * (COALESCE(av.avail,1.0) * COALESCE(gr.growth,1.0)) AS ft_att
  FROM public.player_season_stats s
  LEFT JOIN av ON av.lk = s.loose_key
  LEFT JOIN gr ON gr.lk = s.loose_key
  WHERE s.season = 2026
    AND s.loose_key IS NOT NULL
