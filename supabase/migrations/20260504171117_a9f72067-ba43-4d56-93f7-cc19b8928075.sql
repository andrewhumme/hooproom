...
ALTER TABLE public.draft_rooms
  ADD COLUMN IF NOT EXISTS auction_max_concurrent_nominations smallint NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS auction_nominations_per_team smallint;

-- 2) Validate new fields in existing trigger (extend it)
...
-- 3) Rework next-nominator: snake based on total nominations (active+awarded),
--    skipping teams that are full, that already hold an active nomination, or
--    that have exhausted their quota.
...
begin
  ...
  -- cursor = total nominations created (active + awarded)
...
-- 4) Allow concurrent nominations + enforce per-team rules in nominate RPC.
...
begin
  ...
  -- only one active nomination per team
  select count(*) into _team_active from public.auction_nominations
    where room_id = _room_id and nominator_team_idx = _next_team and status = 'active';
  if _team_active > 0 then
    raise exception 'You already have an active nomination — wait for it to be awarded';
  end if;
