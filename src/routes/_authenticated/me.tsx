...
import { ArrowRight, Clock, Crown, Loader2, Trophy, Users } from "lucide-react";
...
function MyDraftsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      [...((hostedRes.data ?? []) as RoomRow[]), ...joinedRooms].forEach((r) =>
        byId.set(r.id, r),
      );
      if (mounted) {
        setRooms(
  ...
  const hosted = useMemo(() => rooms.filter((r) => r.host_user_id === userId), [rooms, userId]);
  const active = useMemo(() => rooms.filter((r) => r.status !== "complete"), [rooms]);
  const completed = useMemo(() => rooms.filter((r) => r.status === "complete"), [rooms]);
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
          ...
          <TabsContent value="active" className="mt-6">
            <RoomList
              loading={loading}
              rooms={active}
              hostedIds={new Set(hosted.map((r) => r.id))}
              empty={
                <EmptyState
          ...
          <TabsContent value="completed" className="mt-6">
            <RoomList
              ...
              rooms={completed}
              hostedIds={new Set(hosted.map((r) => r.id))}
              empty={<EmptyState title="No completed drafts on your record yet." />}
              onOpen={(id) => navigate({ to: "/draft/$roomId", params: { roomId: id } })}
            />
          </TabsContent>
